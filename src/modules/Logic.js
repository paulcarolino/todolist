...
export function addTask(taskInput, targetProject) {
  ...
  if (taskInput != "") {
    var isValid = !todolist.find(
      (Task) =>
        Task.name === taskInput &&
        Task.projectId === (targetProject ? targetProject.name : "Inbox")
    );
    if (isValid) {
      //Add Task
...
export function findTask(task, targetProject) {
  ...
  if (targetProject) {
    return todolist.find(
      (Task) =>
        Task.name === taskName && Task.getProjectId() == targetProject.getName()
    );
  } else {
    ...
    if (isInInbox) {
      ...
      if (projectName) {
        return todolist.find(
          (Task) => Task.name === taskName && Task.getProjectId() == projectName
        );
      }
    } else {
      return todolist.find(
        (Task) => Task.name === taskName && Task.getProjectId() == "Inbox"
      );
    }
  }
}
...
export function updateTaskName(targetTask, taskName) {
  var todolist = Storage.getTasks();
  targetTask = todolist.find(
    (Task) =>
      Task.name === targetTask.name &&
      Task.getProjectId() == targetTask.projectId
  );
...
export function sortTaskByWeek(todolist) {
  return todolist.filter((task) =>
    isThisWeek(new Date(task.getDate()), { weekStartsOn: 1 })
  );
}
